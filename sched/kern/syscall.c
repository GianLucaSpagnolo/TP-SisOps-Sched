...
// Cantidades de tickets que un env puede poseer
#define MINIMA_CANT_TICKETS 1
#define MAXIMA_CANT_TICKETS 100
...
sys_exofork(void)
{
	...
	newenv->env_priority = curenv->env_priority;  // Prioridad heredada
...
// Devuelve la prioridad de un proceso. Si el envid es 0, devuelve la prioridad del proceso actual.
//
// Devuelve la prioridad del proceso, o -1 en caso de error o si no se encuentra el proceso.
//
static int
...
// Disminuye la prioridad del proceso actual. Esto lo realiza disminuyendo la
// cantidad de tickets que posee. No se le permite a un proceso aumentar la
// prioridad, pero si reducirla.
//
// Devuelve 0 si se realizo correctamente, -1 en caso de error.
//
static int
sys_env_set_priority(int new_priority)
{
	if (new_priority < MINIMA_CANT_TICKETS ||
	    new_priority > curenv->env_priority)
		return -1;

	curenv->env_priority = new_priority;
	return 0;
}

// Aumenta la prioridad de un env, definido por su process_id.
//
// En caso de que el env no se este corriendo, y este listo para ser corrido,
// se le asigna la prioridad deseada incrementando el valor de la cantidad de
// tickets que posee. En caso de que el env se este corriendo, se le disminuye
// la prioridad recibida por parametro.
//
// Si el valor de la prioridad es menor a 1 o mayor a 100, se devuelve -1.
//
// Devuelve 0 si se realizo correctamente, -1 en caso de error o si no se
// encuentra el env.
//
static int
sys_env_change_priority(envid_t envid, int new_priority)
{
	struct Env *env;
	envid2env(envid, &env, 0);

	if (!env || new_priority < MINIMA_CANT_TICKETS ||
	    new_priority > MAXIMA_CANT_TICKETS)
		return -1;

	if (env->env_status == ENV_RUNNING) {
		int old_priority = env->env_priority;
		if (old_priority - new_priority < MINIMA_CANT_TICKETS)
			return -1;

		env->env_priority -= new_priority;
	} else if (env->env_status == ENV_RUNNABLE) {
		int old_priority = env->env_priority;
		if (old_priority + new_priority > MAXIMA_CANT_TICKETS)
			return -1;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_env_get_priority:
		return sys_env_get_priority(
		        a1);  // Devuelve la prioridad de cualquier env
	case SYS_env_set_priority:
		return sys_env_set_priority(
		        a1);  // Setea la prioridad del env actual
	case SYS_env_change_priority:
		return sys_env_change_priority(
		        a1, a2);  // Cambia la prioridad de cualquier env
	default:
		return -E_INVAL;
	}
}
